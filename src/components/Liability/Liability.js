...
const Liability = ({ onAddLiability, liabilities }) => {
  ...
  const handleAddLiability = (event) => {
    ...
    if (liabilityValue !== "") {
      ...
      const liabilityAmount =
        parseFloat(document.getElementById("liability-amount").value) || 0;
      ...
      setLiabilityTotal(liabilityTotal + newLiability.amount)
      //Clear Inputs
  ...
  const getLiabilityTotal = () => {
    if (Array.isArray(liabilities)) {
      return liabilities
        .reduce((total, liability) => total + parseFloat(liability.amount), 0)
        .toFixed(2);
    } else {
      return 0;
    }
  ...
  return (
    <>
      <tr>
        <th>
          <label for="liabilities">Liability Category</label>
        </th>
        <th> Liability Amount</th>
        <th>Current Assets</th>
        <th>Liability Total:${liabilityTotal}</th>
      </tr>
      <tr>
        <td>
